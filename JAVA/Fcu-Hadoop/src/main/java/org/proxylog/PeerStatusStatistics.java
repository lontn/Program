...
public class PeerStatusStatistics {
    private static final Logger L = LoggerFactory.getLogger(PeerStatusStatistics.class);
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Path outputPath = new Path(hdfs.getWorkingDirectory().toString() + "/output/PeerStatusOutput-" + new DateTime().toString("yyyy-MM-dd HH:mm:ss.sss")); //當下的運行的目錄
            Job job = new Job(conf, "PeerStatusStatistics");
    ...
    public static class PeerStatusStatisticsMapper extends TableMapper<Text, IntWritable>{
        ...
        public void map(ImmutableBytesWritable row, Result columns, Context context) throws IOException {
            ...
            try {
                ...
                L.error("PeerStatusStatisticsMapper fail.", e);
                L.error("Row:{}", row.get());
                L.info("peerStatus:{}", peerStatus);
                context.getCounter(ProxyLog.Counters.ERROR).increment(1);
            }
        }
    }
    
    public static class PeerStatusStatisticsReducer extends Reducer<Text, IntWritable, Text, IntWritable> {
