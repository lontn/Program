...
public class ContentTypeStatistics {
    private static final Logger L = LoggerFactory.getLogger(ContentTypeStatistics.class);
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Path outputPath = new Path(hdfs.getWorkingDirectory().toString() + "/output/ContentTypeOutput-" + new DateTime().toString("yyyy-MM-dd HH:mm:ss.sss")); //當下的運行的目錄
            Job job = new Job(conf, "ContentTypeStatistics");
    ...
    public static class ContentTypeStatisticsMapper extends TableMapper<Text, IntWritable>{
...
//            String[] time = timeStemp.split(" ");
            ...
            try {
                ...
                L.error("Row:{}", row.get());
                L.info("contentType:{}", contentType);
                context.getCounter(ProxyLog.Counters.ERROR).increment(1);
            }
        }
    }
    
    public static class ContentTypeStatisticsReducer extends Reducer<Text, IntWritable, Text, IntWritable> {
